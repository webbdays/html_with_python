...
def pytoh(html ,file_name=None, htmlstr = None):
    ...
    """
        for each in html.head:
            if str(each.__class__.__name__) == "ttitle":
                html_str += ("<title>\n")
                html_str += (f"{each.content}\n")
                html_str += ("</title>\n")
        html_str += ("</head>\n")
        ...
        # for body
        html_str += ("<body>\n");
        for each in html.body:
            if str(each.__class__.__name__) == "tp":
                html_str += (f'<p id="{each.id_class.id}" class="{" ".join(each.id_class.classs) }" >\n') # style will be added later
                html_str += (f"{each.content}\n")
                html_str += ("</p>\n")
        html_str += ("</body>\n")
